...
impl AuthBackend for VulnerableAuth {
    ...
    async fn login(&self, form: LoginForm) -> Result<Session, ApiError> {
        ...
        // Güvensiz deseralizasyon ve taklit için session token yerine imzasız base64 çerez de kullanılacak,
        // ancak backend oturumu için de zayıf/basit bir token üretiyoruz:
    ...
    async fn search_posts(&self, query: &str) -> Result<Vec<(Post, String)>, ApiError> {
        ...
        let rows = sqlx::query_as::<_, (i64, i64, String, chrono::DateTime<chrono::Utc>, String)>(&raw_query)
            .fetch_all(&self.pool)
            .await?;
